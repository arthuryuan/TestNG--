...
def get_fixture_desc_class():
   return "TESTNGPPST_NS::TestFixtureDesc"
...
static TESTNGPPST_NS::TestCase* %s[] = {'''
...
static TESTNGPPST_NS::TestFixtureDesc %s
   ( "%s"
   , "%s"
   , %s
...
################################################
suite_desc_template = '''
static TESTNGPPST_NS::TestSuiteDesc %s
   ( "%s"
   , %s
   , (sizeof(%s)/sizeof(%s[0])) - 1
   );
...
extern "C" DLL_EXPORT TESTNGPPST_NS::TestSuiteDesc* %s() {
   return &%s;
}
...
class SuiteGenerator:
   ...
   def generate_dep_headers(self):
      for header in dep_headers:
         output("#include <testngppst/" + header + ">", self.file)
